import { deployments, ethers, network } from 'hardhat'
import { developmentChains } from '../../helper-hardhat-config'
import { DynamicSvgNft, MockV3Aggregator } from '../../typechain-types'
import { SignerWithAddress } from '@nomiclabs/hardhat-ethers/signers'
import { expect } from 'chai'

const lowSVGImageuri =
   'data:image/svg+xml;base64,'
const highSVGimageUri =
   'data:image/svg+xml;base64,'

const highTokenUri =
   'data:application/json;base64,'
const lowTokenUri =
   'data:application/json;base64,'

developmentChains.includes(network.name) &&
   describe('DynamicSvgNft Unit Tests', function () {
      let dynamicSvgNft: DynamicSvgNft
      let mockV3Aggregator: MockV3Aggregator
      let deployer: SignerWithAddress

      beforeEach(async function () {
         const accounts = await ethers.getSigners()
         deployer = accounts[0]
         await deployments.fixture(['mocks', 'dynamicsvgnft'])
         dynamicSvgNft = await ethers.getContract('DynamicSvgNft', deployer)
         mockV3Aggregator = await ethers.getContract(
            'MockV3Aggregator',
            deployer
         )
      })

      describe('constructor', function () {
         it('initializes priceFeed correctly', async function () {
            const priceFeed = await dynamicSvgNft.getPriceFeed()
            expect(priceFeed).to.equal(mockV3Aggregator.address)
         })

         it('initializes image URIs correctly', async function () {
            const lowImageURI = await dynamicSvgNft.getLowSVG()
            const highImageURI = await dynamicSvgNft.getHighSVG()
            expect(lowImageURI).to.equal(lowSVGImageuri)
            expect(highImageURI).to.equal(highSVGimageUri)
         })
      })

      describe('mintNft', function () {
         const highValue: string = ethers.utils.parseEther('1800').toString()

         it('assignes nft id to highValue correctly', async function () {
            const id = await dynamicSvgNft.getTokenCounter()
            await dynamicSvgNft.mintNft(highValue)
            const hValue = await dynamicSvgNft.getHighValue(id)
            expect(hValue.toString()).to.equal(highValue)
         })

         it('assignes nft id to signer correctly', async function () {
            const id = await dynamicSvgNft.getTokenCounter()
            await dynamicSvgNft.mintNft(highValue)
            const owner = await dynamicSvgNft.ownerOf(id)
            expect(owner.toString()).to.equal(deployer.address)
         })

         it('increments tokenCounter', async function () {
            await dynamicSvgNft.mintNft(highValue)
            const id = await dynamicSvgNft.getTokenCounter()
            expect(id.toString()).to.equal('1')
         })

         it('emits CreatedNft event', async function () {
            await expect(dynamicSvgNft.mintNft(highValue)).to.be.emit(
               dynamicSvgNft,
               'CreatedNft'
            )
         })
      })

      describe('svgToImageURI', function () {
         it('returns correct URI', async function () {
            const svg = '<svg></svg>'
            const imageURI = await dynamicSvgNft.svgToImageURI(svg)
            expect(imageURI).to.include('data:image/svg+xml;base64,')
         })
      })

      describe('tokenURI', function () {
         it(`reverts if tokenId doesn't exist`, async function () {
            await expect(
               dynamicSvgNft.tokenURI(6)
            ).to.be.revertedWithCustomError(
               dynamicSvgNft,
               'ERC721Metadata__URI_QueryFor_NonExistentToken'
            )
         })

         it('returns highTokenUri if price is higher than expected', async function () {
            const highValue: string = ethers.utils.parseEther('1').toString()
            const tx = await dynamicSvgNft.mintNft(highValue)
            await tx.wait(1)
            const tokenURI = await dynamicSvgNft.tokenURI(0)
            expect(tokenURI).to.equal(highTokenUri)
         })

         it('returns lowTokenUri if price is lower than expected', async function () {
            const highValue: string = ethers.utils
               .parseEther('1000000')
               .toString()
            const tx = await dynamicSvgNft.mintNft(highValue)
            await tx.wait(1)
            const tokenURI = await dynamicSvgNft.tokenURI(0)
            expect(tokenURI).to.equal(lowTokenUri)
         })
      })
   })
